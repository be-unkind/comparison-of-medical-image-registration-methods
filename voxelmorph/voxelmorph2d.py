...
class UNet(nn.Module):
    ...
    def __init__(self, in_channel, out_channel):
        ...
        self.dropout = torch.nn.Dropout2d(.3)
        # Bottleneck
        mid_channel = 128
    ...
    def crop_and_concat(self, upsampled, bypass):
        # Ensure that upsampled and bypass have the same spatial dimensions
        target_size = upsampled.shape[2:]
        bypass = self.crop(bypass, target_size)
    ...
    def crop(self, layer, target_size):
        batch_size, num_channels, layer_width, layer_height = layer.size()
        target_width, target_height = target_size

        # Compute cropping parameters
        x1 = int((layer_width - target_width) / 2)
        y1 = int((layer_height - target_height) / 2)
        x2 = x1 + target_width
        y2 = y1 + target_height

        # Crop the layer
        return layer[:, :, x1:x2, y1:y2]

    def forward(self, x):
        # Encode
        encode_block1 = self.conv_encode1(x)
        encode_pool1 = self.dropout(self.conv_maxpool1(encode_block1))
        encode_block2 = self.conv_encode2(encode_pool1)
        encode_pool2 = self.dropout(self.conv_maxpool2(encode_block2))
        encode_block3 = self.conv_encode3(encode_pool2)
        encode_pool3 = self.dropout(self.conv_maxpool3(encode_block3))
        # Bottleneck
        bottleneck1 = self.bottleneck(encode_pool3)
        # Decode
        decode_block3 = self.dropout(self.crop_and_concat(bottleneck1, encode_block3))
        cat_layer2 = self.conv_decode3(decode_block3)
        decode_block2 = self.dropout(self.crop_and_concat(cat_layer2, encode_block2))
        cat_layer1 = self.conv_decode2(decode_block2)
        decode_block1 = self.dropout(self.crop_and_concat(cat_layer1, encode_block1))
...
###ADDED
def print_image_range(I):
    min_val = torch.min(I)
    max_val = torch.max(I)
    print("Image range: [{}, {}]".format(min_val.item(), max_val.item()))
...
def cross_correlation_loss(I, J, n):
    ...
    I_sum = torch.conv2d(I, sum_filter, padding=padding, stride=(1,1))
    J_sum = torch.conv2d(J, sum_filter,  padding=padding ,stride=(1,1))
    I2_sum = torch.conv2d(I2, sum_filter, padding=padding, stride=(1,1))
    J2_sum = torch.conv2d(J2, sum_filter, padding=padding, stride=(1,1))
    IJ_sum = torch.conv2d(IJ, sum_filter, padding=padding, stride=(1,1))
...
#     I_centered = I - I_mean
#     J_centered = J - J_mean
#     cc = torch.sum(I_centered * J_centered, dim=(2, 3)) / (xdim * ydim)

#     I_var = torch.sum(I_centered ** 2, dim=(2, 3)) / (xdim * ydim)
#     J_var = torch.sum(J_centered ** 2, dim=(2, 3)) / (xdim * ydim)

#     eps = torch.finfo(I_var.dtype).eps
#     cc_normalized = cc / torch.sqrt((I_var + eps) * (J_var + eps))

#     loss = torch.mean(cc_normalized)
...
def vox_morph_loss(y, ytrue, n=9, lamda=0.01):
    ...
    # print("CC Loss", cc.data, "Gradient Loss", sm.data)
