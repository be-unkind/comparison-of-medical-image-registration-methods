...
def _load_data_from_array(data_array, dtype=np.float32, **kwargs):
...
class Trainer(object):
    ...
    def __init__(self, input_size=(96, 96, 96), num_channels=1, num_classes=2,
        ...
        with tf.compat.v1.variable_scope('spatial_transformation'):
            ...
            self.vector_fields = tf.compat.v1.get_variable('vector_fields', shape=[1, input_size[0], input_size[1],
                                                                         input_size[2], 3],
    ...
    def _get_optimizer(self):
        ...
        with tf.name_scope('optimizer'):
            ...
            ### ADDED
            # new_shape = [target_height, target_width]  # Define the new shape with the desired height and width
            # corres_fields = tf.squeeze(corres_fields, axis=0)  # Remove the batch dimension
            # corres_fields = tf.image.resize(corres_fields, new_shape, method='nearest')
            # corres_fields = tf.expand_dims(corres_fields, axis=0)
    ...
    def save(self, sess, model_path, latest_filename=None, **kwargs):
        ...
        saver = tf.compat.v1.train.Saver(**kwargs)
    ...
    def train(self, data, training_iters, display_steps=1, restore=False, model_path='model_trained',
        ...
        with tf.compat.v1.Session(config=config) as sess:
            ...
            for step in range(training_iters):
                ...
                self.final_bending_energy = bending_energy
                self.final_max_jacobian = max_jacobian
                self.final_min_jacobian = min_jacobian
